...
describe("admin-api list query params", () => {
  ...
  /**
   * Every one of these 13 resources is backed by the backend's `crudRouter()` factory
   * (`app/server/routes/admin-features/_crud.ts`), whose `GET`/`POST` handlers are
   * registered at `path: '/'` — the same strict-router mechanism documented on `listAdmins`
   * above (and the one that actually broke `listAdmins` in production before that fix). A
   * trailing slash 404s against the real backend even though it "passes" a unit test that
   * only mocks `fetch` and never exercises Hono's router — so every `list*`/`create*` pair
   * generated by `listAdminResource`/`createAdminResource` gets its own exact-path
   * assertion here rather than trusting a single representative sample.
   */
  describe("listAdminResource-backed CRUD resource paths (no trailing slash)", () => {
    const resources: Array<{
      label: string;
      path: string;
      list: (params?: { limit?: number; skip?: number }) => Promise<unknown[]>;
      create: (payload: AdminResourcePayload) => Promise<unknown>;
    }> = [
      {
        label: "service definitions",
        path: "service-definitions",
        list: listServiceDefinitions,
        create: createServiceDefinition,
      },
      { label: "add-ons", path: "add-ons", list: listAddOns, create: createAddOn },
      { label: "pricing rules", path: "pricing-rules", list: listPricingRules, create: createPricingRule },
      { label: "service areas", path: "service-areas", list: listServiceAreas, create: createServiceArea },
      { label: "promo codes", path: "promo-codes", list: listPromoCodes, create: createPromoCode },
      {
        label: "concierge bookings",
        path: "concierge-bookings",
        list: listConciergeBookings,
        create: createConciergeBooking,
      },
      {
        label: "chat interventions",
        path: "chat-interventions",
        list: listChatInterventions,
        create: createChatIntervention,
      },
      { label: "claim reviews", path: "claim-reviews", list: listClaimReviews, create: createClaimReview },
      { label: "service credits", path: "service-credits", list: listServiceCredits, create: createServiceCredit },
      {
        label: "payout adjustments",
        path: "payout-adjustments",
      ...
      { label: "cleaner tags", path: "cleaner-tags", list: listCleanerTags, create: createCleanerTag },
      {
        ...
        list: listAvailabilityOverrides,
        create: createAvailabilityOverride,
      },
    ];

    for (const resource of resources) {
      it(`${resource.label}: list hits /api/v1/admins/${resource.path} with no trailing slash before the query string`, async () => {
        const fetchMock = getFetchMock();
        fetchMock.mockResolvedValueOnce(jsonResponse(envelope({ items: [], total: 0 })));

        await resource.list();

        expect(fetchMock.mock.calls[0][0]).toBe(`/api/v1/admins/${resource.path}?limit=100&skip=0`);
      });

      it(`${resource.label}: create posts to /api/v1/admins/${resource.path} with no trailing slash`, async () => {
        const fetchMock = getFetchMock();
        fetchMock.mockResolvedValueOnce(jsonResponse(envelope({ id: "new1" }), 201));

        await resource.create({ name: "x" });

        expect(fetchMock.mock.calls[0][0]).toBe(`/api/v1/admins/${resource.path}`);
